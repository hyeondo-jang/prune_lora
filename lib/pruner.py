...
class ALPS_prune:
    ...
    def ALPS_admm(self, sp, nm_n = 0, nm_m = 0, rho=0.1, max_iter = 300, update_iter = 3, switch_iter = 30, fix_supp=False):
        ...
        self.YtX = torch.matmul(W.cpu() * X_norm,self.XtX).to(self.dev)
        ...
        for i_admm in range(max_iter):
            ...
            if (i_admm+1) % update_iter == 0:
                ...
                Resc = torch.matmul(self.XtX.to(self.dev),Btest) - self.YtX.T
        ...
        back_st = time.time()
        B = self.cg_batch( (self.XtX).to(self.dev), self.YtX.T, 
